...
interface EmployeApiResponse { id:number;username:string;nom:string;prenom:string;email:string;telephone:string|null;adresse:string|null;dateNaissance:string|null;dateEmbauche:string|null;sexe:string|null;photoUrl?:string|null;changementMotDePasseRequis?:boolean;actif:boolean;role?:RoleType;statut?:string;poste:{id:number;intitule:string}|null;typeContrat:{id:number;libelle:string}|null;manager:{id:number;nom:string;prenom:string;email:string}|null;createdAt:string;updatedAt:string|null; }
interface EmployeApiRequest { username:string;nom:string;prenom:string;email:string;telephone:string|null;adresse:string|null;dateNaissance:string|null;dateEmbauche:string|null;sexe:string|null;actif:boolean;role:RoleType;posteId:number|null;typeContratId:number|null;managerId:number|null; }

@Injectable({providedIn:'root'})
export class UserService {
 private readonly http=inject(HttpClient);private readonly baseUrl=`${environment.apiUrl}/employes`;
